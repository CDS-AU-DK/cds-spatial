---
title: "Distances, Travel and Accessibility"
...
## Analyzing Accessibility - Aarhus Hospital 
...
## Accessibility of Hospitals in Midtjylland
...
mapbox_map %>%
  addPolygons(data = walking_isos,
              popup = ~id) %>% 
  addPolygons(data=cycling_isos, color = "red") %>% 
   addPolygons(data=driving_isos, color = "yellow")
...
* First read in the type and count of households ("husstande-og-familier") from 2020 and 2021 from [DS](https://www.dst.dk/en/Statistik/emner/borgere/husstande-familier-og-boern/husstande-og-familier) . Beware that the dataset has no headers, and read.csv() might perform better than tidyverse counterpar.
* Summarize this by type of household so you see what it contains
...
* Join up this dataset with no-car dataset with inner or `left_join()` and calculate percentages of households without a car in 2020 and 2021.
* Join the result with the `municipalities` sf object to add the numbers to a map
* Create a thematic map of the municipalities showing the percentage of households without a car per each municipality (I recommend mapview(), but you are free to use your favorite library)
...
* transform the coordinate reference system of our no_car_pct_mun dataset to 4326, the same CRS used by the isochrones;
* extract only those municipalities with a percentage of households without cars of 25 percent or above;
* use the st_difference() function to “cut out” areas from those municipalities that overlap the 20-minute walking and cycling isochrones respectively.
* Once we complete this operation, we can visualize the result on our Mapbox map.

```{r}
## Suggested progress
target_areas <- no_cars_pct_mun %>%
  st_transform(4326) %>%
  filter(estimate >= 15) %>%
  st_difference(
...
1. Are there any areas that are located beyond a 20-minute walk or bikeride of a hospital that have proportionally lower access to cars? Where and what is their area?
2. What would we need to know to ensure residents in these areas have reasonable access to hospitals?
