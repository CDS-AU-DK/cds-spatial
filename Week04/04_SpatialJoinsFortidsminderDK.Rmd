...
plot(municipalities_25832%>%
      filter(NAME_2%in%least$NAME_2) 
      %>% st_geometry(), col = "red", add = T)
```
...
* Extract `viking` and `parks` that intersect and are contained by  `nobel_buf` and save as `int` and `cont`. You should extract the first element of the `int` and `cont` object to be able to get the names of the viking monuments thereafter.
...
nb <- data.frame(name = "Nobel", long = 10.2033485,lat = 56.174346)
nb_buf <- nb %>% 
...
# Get in the nobel object and viking_clip
...
```{r}
# /Start Code/ #
A <- municipalities_25832 %>% filter(NAME_2 == "Århus")
library(tmap)
...
tm_shape(A) + 
  ...
  tm_shape(viking)+
  tm_dots(col= "anlaeg_simple")

head(viking)
unique(viking$anlaegstyp)
viking$anlaeg_simple <- str_extract(viking$anlaegstyp,"^\\d{3}")
# /End Code/ #
```
