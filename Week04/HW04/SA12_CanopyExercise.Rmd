...
* Load `sf` and `dplyr` in your workspace.
...
3. Combine the `manhattan` and `neighborhoods` maps above with a `+` and save this as `map1` (the `neighborhoods` layer/map should be second map object `map2`. Remember to use quantile style and an appropriate palette).
...
Congratulations! Hopefully you've created a nice final map you can share with colleagues to show how average tree canopy is a better representation of green space than the tree density based on the Street Tree Census data from NYC.