...
# Data
mounds <- st_read("../scripts_rs/data/KAZ_mounds.shp")
boundary <- st_read("../scripts_rs/data/KAZ_surveyarea.shp")
...
testnnd <- nndist(moundsp)
hist(testnnd)

testG <- Gest(moundsp)
plot(testG)

testK <- Kest(moundsp, correction = 'border')
plot(testK)
...
?st_simplify()
bound <- st_simplify(boundary, dTolerance = 100)
b <- st_union(bound)
plot(b)
...
?as.ppp()
?owin()
mounds_ppp <- ppp(mounds_int, window = hull)
window <- as.owin(hull_sm)
window <- as.owin(as_Spatial(hull_sm))

# Errorring out? Try this vignette
...
mounds_int_ppp <- ppp(x,y, window = window) 
...
head(mounds)
data <- read_csv("../scripts_rs/data/KAZ_mdata.csv")
head(data)
tail(data)
...
mounds_ppp <- as.ppp(st_coordinates(m_marks), W=w)
plot(mounds_ppp)
mounds_ppp

#marks(m_ppp)<- m_marks[,10:11]
marks(mounds_ppp) <- m_marks$ConditionBin # assigning marks by vector
 # works better than by dataframe, or perhaps it is happy with a single mark at a time

##################################### Spatial Segregation

mounds_robbed <- split(m_ppp, "Robbed2")
plot(mounds_robbed)
...
# best kernel bandwidth for moundsp is 900
