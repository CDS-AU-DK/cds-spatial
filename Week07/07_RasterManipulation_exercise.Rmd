...
Raster data can be very big depending on the extent and resolution (grid size). In order to deal with this the `rast()` function is designed to only read in the actual raster values as needed. To show that this is true, you can use the `inMemory()` function on an object and it will return `FALSE` if the values are not in memory. If you use the `head()` function, the `terra` package will read in only the values needed, not the full set of values. The raster values will be read in by default if you perform spatial analysis operations that require it or you can read in the values from a raster manually with the function `getValues()`.


## Instructions

* Activate `terra` library
* Read in the Aster image. Review Week 02 raster loading if unsure about how. Inspect its extent and metadata.
...
Congratulations! You now know that the `terra` package only reads in raster values as needed to save space in memory. You can get the values manually using the `getValues()` function.
...
There are many situations where you might need to recode raster values. You may want to change the outlier values to `NA` for example. In the `terra` package, reclassification is performed with the `classify()` function.
...
* Use the matrix and `classify()` to reclassify values below 0 to `NA`. You will need to use the argument `rcl`.
...
# Set up the matrix [from, to, becomes]
vals <- ___
...
* Aggregate the IKONOS image using the default for `fun` and with a `fact` of 10 and save the new raster to `east_small`.
...
* Use the `terra` function `extract()` to determine the elevation at each of the mounds. Assign the extracted value into a `elev` column in the mounds object. Beware that the `extract()` function exists across multiple packages, so it's wise to use `terra::` in front of it.
* The result of `terra::extract()` is usually a data frame where the first column is usually the ID and the second is the extracted value. Wrangle it to be a column in mounds.
...
What you will do in this exercise is essentially: identify the most prominent locations among the registered archaeological mounds by finding areas that have both a high percentage of prominence and a high elevation. You will use two rasters and raster math to intersect elevations between 400 and 650 masl with prominence over 60%. Remember that raster algebra uses basic Boolean logic to select desirable values. Note that you can only do raster math on two rasters of the same *extent*, so make sure you align the elevation and prominence accordingly with the `crop()` function
...
* Use the boolean operators `&`, `|` to specify the elevation band of 450-600 masl and greater than 60% prominence.
* Plot the result.
...
elevation
prominence <- _____("_____")

# Plot prominence on top of elevation

# Align the extent of the two rasters with crop()

# Do the overlay using the boolean operators.

# Plot the result (low elevation and high prominence areas)
plot(prom_el_overlay)
```

Congratulations! You've now learned to perform raster math using the standard arithmetic/logical operations. You limited to areas with >400m & <650m elevation and >60% prominence, these areas should harbor the most prominent mounds (or defense-ready areas if associated with settlements) in Kazanlak.
...
# Task 7: Find the most prominent mounds
...
*5. How can you test if the mounds fall in higher elevation/prominence locations than expected by chance? *
