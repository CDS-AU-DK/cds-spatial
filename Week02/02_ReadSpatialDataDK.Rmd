...
# View the first shelters, and forests
head(shelters)
head(forests)
head(parks)
shelters
...
### Solution
```{r wrangle-parks-sol, echo=FALSE}
# Load the sf package
library(sf)
...
# Limit columns to navn and areal_m2 variables
over5_lim <- over5ha %>% dplyr::select(navn, areal_m2) 

# Use head() to look at the first few records
plot(over5_lim$geometry)
```
...
# Can you plot the big_forests geometry and their names?(hint: check the plotting section in Task 5 below)
big_forests$navn
...
# Plot the forests object using all defaults
plot(parks$geometry); plot(forests$geometry, add = TRUE)

# Plot just the Area_ha attribute of the forests data
plot(parks["areal_m2"]); plot(forests["areal_m2"], add = TRUE)
...
# Plot the geometry of the forests and the parks object together
plot(parks$geometry, col = "red"); plot(forests$geometry, col = "green", add = TRUE)
```
...
# Get the CRS of the aarhus object
crs(aarhus)
crs(elevation)
...
# Plot the two images together (on top of one another)
...
# Assign the CRS to playgrounds - its CRS is actually 4326, it just got lost in transport
crs_1 <- 4326
...
st_crs(playgrounds_crs) == crs(aarhus)


# # See if aarhus and playgrounds share a CRS
# st_crs(playgrounds)
# crs(aarhus)
# 
# # Save the CRS of the aarhus layer
# the_crs <- crs(aarhus, asText = TRUE)
# 
# # Transform the playgrounds CRS to match aarhus
# playgrounds_crs <- st_transform(playgrounds, crs = the_crs)
# 
# # Re-run plotting code (run both lines together)
# # Do the playgrounds show up now?
# plot(aarhus);plot(playgrounds_crs, add = TRUE)
