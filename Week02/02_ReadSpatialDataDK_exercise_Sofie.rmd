...
OBS: Ida Denckers markdown, as I didn't participate Monday 13/2
...
1. How many features (rows) does each layer contain and what kind of geometry are they?

  forest: 32, miltipolygon
  parks:47, miltipolygon
  playgrounds: 51, point
  shelters: 19, point
  
2. What is the CRS value in these objects?

  ''Projected CRS: ETRS89 / UTM zone 32N'', in playgrounds it is NA 
...
head(over5_lim)
#the geometry column is kept to..
```
...
#7 parks where area is over 100000
```
...
plot(big_forests$geometry, col = "green")
...
plotRGB(aarhus, stretch= 'lin');plot(elevation, add =TRUE)
#choose the map with the biggest extent first, it will be the baseline
#doesnt really look so nice
...
st_crs(forests) #ETRS89 / UTM zone 32N 
...
plotRGB(aarhus, stretch = "lin")
plot(playgrounds, add = TRUE) #If run together the playground dont appear

#Try the playground projected, 
#earlier we created this code: playgrounds_crs <- st_transform(playgrounds, crs = the_crs)
#so we can use the playgrounds_crs
plotRGB(aarhus, stretch = "lin")
plot(playgrounds_crs, add = TRUE)
#Now we se the playgorunds as little dots

#Green larger dots
plotRGB(aarhus, stretch = "lin")
plot(playgrounds_crs, col= "green", lwd= 3, alpha=100, add = TRUE)
...
plot(municipalities) #Kommuner
plot(DK_border) #ydre grænse
```
