...
species_counts <- ___(trees, species, sort = ___)

# Use head to see if the geometry column is in the data frame
___(species_counts)

# Drop the geometry column
species_no_geometry <- ___(species_counts, ___)
...
boro <- ___(neighborhoods, boro_name, county_fip, boro_code)

# Drop the geometry column
boro_no_geometry <- ___(___, ___)

# Limit to distinct records
boro_distinct <- ___(boro_no_geometry)

# Join the county detail into the trees object
trees_with_county <- ___(trees, boro_distinct, by = c("boroname" = "boro_name"))

# Confirm the new fields county_fip and boro_code exist
___(trees_with_county)
```
...
pts_neighborhoods <- st_cast(___$geometry, "MULTIPOINT")
cnt_neighborhoods <- sapply(pts_neighborhoods, length)
sum(cnt_neighborhoods)

# Simplify the neighborhoods object
neighborhoods_simple <- ___(neighborhoods, 
                            preserveTopology = ___, 
                            dTolerance = ___)

# Measure the size of the neighborhoods_simple object
___(neighborhoods_simple)

# Compute the number of vertices in the neighborhoods_simple object
pts_neighborhoods_simple <- st_cast(___, "MULTIPOINT")
cnt_neighborhoods_simple <- sapply(pts_neighborhoods_simple, ___)
...
trees_sf <- ___(trees, coords = ___, crs = ___)
