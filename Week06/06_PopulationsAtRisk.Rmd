...
# load household data
hh <- read.csv2("../data/households2020_2021.csv", header = FALSE, fileEncoding="windows-1252")
...
pop <-  read.csv2("../data/munic_pop_2022Q1.csv", sep = ",", skip = 2, fileEncoding="windows-1252")
...
# check that the names are spelled the same between hh_car, and munic dataset
#munic$NAME_2[munic$NAME_2%nin%hh_nocar$municipality] 

# [1] "Høje Taastrup"  "København"      "Århus"          "Vesthimmerland"

hh_nocar$municipality[3] <- "Århus"
hh_nocar$municipality[44] <- "Høje Taastrup"
hh_nocar$municipality[100] <- "Vesthimmerland"
#hh_nocar$municipality[3] <- "Århus"
...
* Use this single region to clip the municipalities data on carless households `no_cars_pct_mun`
...
```{r thematic-map-Midt-sol, echo = FALSE}
...
# intersect data with municipalities
...
cycling_isos <- readRDS("../../cds-spatial-2023/data/cycling_isos.rds")

## Check what falls out during 25% carless household cutoff
no_cars_pct_midt %>% 
  filter(nc2021pct >25) %>% 
  mapview() + mapview(cycling_isos)

## Find target areas (union cycling isos)
...
mapview(st_sf(target25pct))

# Calculate area on projected 2D..
area_proj <- st_area(st_transform(target25pct, crs = 25832))/1000000

# ..and on 3D target area
area_3d <- st_area(target25pct)/1000000 # beware that the units will show as m2 even though they are km2
