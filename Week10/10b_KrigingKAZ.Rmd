...
km <- krige(OM ~ ___ + ___, kaz_geo, newdata = kaz_geo_miss, model = v_model)
names(km)
...
grid <- ___(___, c(500, 500), c(35, 40))
...
OM_grid <- ___(OM ~ x + y, kaz_geo, newdata = ___, model = ___)
...
OM_grid$pFertile <- 1 - pnorm(7, mean = OM_grid$___, sd = ___(OM_grid$___))

# Map the probability of soils OM exceeding 7
spplot(OM_grid, zcol = "___")
...
OM_auto_grid <- ___(___, input_data = ___, new_data = ___)
...
Lovely result! The scale in inverse (dark areas show high organic-matter content) but otherwise the result is practically identical to the plot you created manually in task 3 above. 
