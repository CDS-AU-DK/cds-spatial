...
#Repeat for clustered point data
nnd_clustered <- nndist(p_cluster)
hist(nnd_clustered)
...
K_pois_env <- envelope(p_poisson, Kest, correction = "border")

# Insert the full formula to plot K minus pi * r^2
plot(K_pois_env, . - pi * r ^ 2 ~ r)
...
nn_red <- nndist(redoak)
hist(nn_red)
g_red <- Gest(redoak, correction = "border")
plot(g_red)
k_red <- Kest(redoak, correction = "border")
plot(k_red)
k_env <- envelope(redoak, Kest, correction = "border")
plot(k_env, . - pi * r ^ 2 ~ r)
```
