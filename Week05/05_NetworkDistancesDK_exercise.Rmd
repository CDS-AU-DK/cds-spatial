...
```{r geocode}
nobel <- mb_geocode("Jens Chr.Skous Vej 5, Aarhus, 8000, Denmark")
nobel
```
...
```{r nobel-drive, eval = FALSE}
nobel_iso_drive <- ______________(nobel,
                                 profile = "__________",
                                 time = c(4, 8, 12))
...
```{r route-moesgaard, eval = FALSE}
route <- ____________(origin = nobel,
...
route_dir <- mb_directions(origin = nobel,
                       destination = c(10.222426021218107,
                                       56.091501015374654), # "Moesgaard Museum, Højbjerg, Denmark",
                       profile = "cycling",
                           steps = TRUE)
...
How many and which Viking monuments/pubs/service stations are reachable within each of the three bands?
```{r}
## YOUR CODE HERE
```
...
# Transform 'isos' to 25832
isos_32 <- _______(isos, "EPSG:25832")

# Create a RasterLayer (SpatRaster in terra) with the specified resolution of 100m
template <- _____(isos_32, resolution = 100)

# Use the rasterize function from terra, specifying the field and aggregation function
iso_surface <- _________(isos_32, template, field = "time", fun = min, background=NA)
...
# library(terra)
# ?writeRaster()
# writeRaster(iso_surface, "iso_surface100.tif")
