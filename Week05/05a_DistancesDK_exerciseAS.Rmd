...
saveRDS(viking, "../data/viking.rds")
st_write(viking, "../data/viking.geojson")

read_sf("../data/viking.geojson")
readRDS("../data/viking.rds")

# Load the viking dataset
#viking_aarhus <- readRDS("../data/viking.rds")
viking_aarhus <- viking


aarhus <- municipalities_25832 %>% 
  filter(NAME_2== "Århus") 

viking_aarhus <-  viking %>% 
  st_intersection(aarhus)


# Read in the nobel object and reconcile crs
nobel <- data.frame(name = "nobel", longitude = 10.2033485,latitude = 56.174346)

nobel <- nobel %>% 
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>% 
  st_transform(df_sf[3,], crs = 25832)

nb_buf <- st_buffer(nobel, dist = 5000)

# Test whether the CRS match
all.equal(crs(nobel), crs(viking))
