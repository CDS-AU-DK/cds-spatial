...
leaflet() %>%
  ...
  addPolygons(data =nobel_iso_drive, 
              ...
              opacity = 0.5, 
              weight = 0.2) %>% 
  addCircleMarkers(data = nobel_viking) %>% 
  addLegend(labels = c(4, 8, 12),
            colors = colors,
            title = "Drive-time<br/>around Nobel")

st_intersects(st_transform(viking_aarhus, 4326), nobel_iso_drive)

overlap_list <- st_intersects(nobel_iso_drive, st_transform(viking_aarhus, 4326))
overlap_list

nobel_viking <- st_intersection(st_transform(viking_aarhus, 4326), nobel_iso_drive)
```
...
route_fisk <- mb_directions(origin = nobel,
                       destination = "Fiskehuset Norsminde, Norsminde, Denmark",
                       profile = "cycling")
route_fisk

mapbox_map %>%
  addPolylines(data = route_fisk, 
               popup = paste0(
                 "Distance (km): ",
                 round(route_fisk$distance, 1), 
                 "<br/>Time (minutes): ",
                 round(route_fisk$duration, 1)
               ))
```
